...
var ResourcePool = (function () {
    ...
    ResourcePool.createOneByType = function (type, resId) {
        if (resId == ResourcePool.Trace_ID)
            Log.trace("ResourcePool . {0} : {1}", "createOneByType", resId);
        switch (type) {
        ...
        Log.trace("Can't Create Instance of : " + type + "with ResId: " + resId);
    };
        ...
        switch (type) {
            case ResourcePool.SKE:
                if (item instanceof BoneAnimation) {
                    ...
                    Log.trace("item type not match!!" + item.name + "  type :" + type);
                }
                break;
        }
    };
    ...
    */
    ResourcePool.prototype.pop = function (resId, type) {
        ...
        if (resId == ResourcePool.Trace_ID)
            Log.trace("ResourcePool . {0} : {1}", "pop", resId);
        var list = pool[resId];
        if (list.length > 0) {
            var item = list.pop();
    ...
    ResourcePool.prototype.push = function (obj, type) {
        ...
        if (obj.name == ResourcePool.Trace_ID)
            Log.trace("ResourcePool . {0} : {1}", "push", obj.name);
    ...
    ResourcePool.prototype.create = function (resId, type, num) {
        ...
        var list = [];
        if (resId == ResourcePool.Trace_ID)
            Log.trace("ResourcePool . {0} : {1}  {2}", "create", resId, num);
        for (i = 0; i < num; i++) {
    ...
    * @type 清理前执行指定函数
    ...
    ResourcePool.prototype.clear = function (resId, type, funName) {
        ...
        var pool = this.getPoolByType(type);
        if (resId == ResourcePool.Trace_ID)
            Log.trace("ResourcePool . {0} : {1}  {2}", "clear", resId, type);
        if (resId == "") {
            for (var key in pool) {
                clearItem(pool[key]);
            }
    ...
    ResourcePool.prototype.printAll = function () {
        ...
        for (var key in this.main) {
            ...
            for (var pKey in this.main[key]) {
                ...
                Log.trace("key :" + pKey + "  count:" + length_1);
                allCount += length_1;
            }
        }
